...
class MetaFlexer {
    ...
    public function generateApp($generationFolderPath, $deletePreviousFiles = true, $stage) {
        ...
        // 2.2 Run the generative Loop
        $this->_allLoop($project); // All loop first
     ...
     */
    private function _allLoop(ProjectRepresentation $project) {
        ...
        foreach ($this->allTemplatesDescriptor as $descriptor) {
            ...
            $this->_cleanUpInjectionsTags($flexed);

        }
    }

    private function _cleanUpInjectionsTags(Flexed $flexed){
        // Remove the //FLEXIONS_TAG_00X
        $source = $flexed->source;
        $intCounter = 1;
        $counter = '001';
        while (strpos($source,'//FLEXIONS_TAG_'.$counter) != false ){
            $tag = '//FLEXIONS_TAG_'.$counter;
            $source=str_replace($tag,'',$source);
            $intCounter ++;
            $counter = str_pad("$intCounter",3,"0",STR_PAD_LEFT);
        }
        $flexed->source = $source;
    }
    ...
    public function setTemplates($templates) {
        ...
        if (is_array($this->_templates[BJM_LOOP_PATHS]) && array_key_exists(BJM_PATH, $this->_templates[BJM_LOOP_PATHS])) {
            ...
            throw  new Exception('Paths meta template is undefined. The json descriptor should contains "root.templates.' . BJM_LOOP_PATHS . '.' . BJM_PATH . '"');
        }
        $templatesMap = [
            ...
            [BJM_LOOP_ALL, 'allTemplatesDescriptor']
        ];
            ...
            if (is_array($loopTemplates)) {
                foreach ($loopTemplates as $loopTemplate) {
                    if (is_array($loopTemplate) && array_key_exists(BJM_PATH, $loopTemplate)) {
                        ...
                        // We don't want to add muliple occurences of the same template.
                        // Excepted if a specific UID as been declared;
                        // So we use the Path of the template + the declared BJM_UID if there is one.
                        $uid = $loopTemplate[BJM_PATH];
                        if (array_key_exists(BJM_UID,$loopTemplate)) {
                            $uid = $loopTemplate[BJM_PATH].$loopTemplate[BJM_UID];
                        }
                        $this->{$tplDescPropertyName}[$uid] = $loopTemplate;
                    } else {
                        throw  new Exception('Path is undefined. The json descriptor should contains "root.templates.' . $key . '.*.path"');
                    }
