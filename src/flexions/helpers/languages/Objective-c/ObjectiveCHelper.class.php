...
class ObjectiveCHelper {
	 ...
	 */
	 ...
	 /*
	  * we do expose primitive types for documentation purposes
	  * to perform metaprogramming more easily
	  */ 
	 ...
	 public function getPrimitiveTypes (){
	 	return array("string","object","array","dictionary","null","any","uint","int","int 16","int 32","int 64","unsigned int","long long","unsigned long","unsigned long long","integer","float","double","bool","number","numeric","rect","size","point");
	 }
	 ...
	 public function getPrimitiveNativeTypes (){
	 	return array("NSString","NSObject","NSArray","NSDictionary","NSNull","id","NSInteger","float","double","BOOL","NSNumber","GGRect","CGSize","CGPoint");
	 }
	 
	/**
	 * Returns the property dŽclaration
	 * @param PropertyRepresentation $property
	 * @param bool $allowScalars used for example when generating for core data
	 * @return string
	 */
...
/**
 * 
 * @param string $expr for example :"self.propertyName"
 ...
 */
	public function objectFromExpression($expr,$type){
		...
		+ (NSNumber *)numberWithInt:(int)value; // We use NSInteger and NSUInteger
	...
	function valueStringForProperty($valueString, PropertyRepresentation $p){
		if($p->isGeneratedType==true){
			return "[".$p->instanceOf. " instanceFromDictionary:$valueString]";
		}
		$nativeType=$this->nativeTypeForProperty($p);
		/*
		 NOT USEFULL
		if($nativeType=='CGRect')
			return  "[$valueString  CGRectValue]";
		
		if($nativeType=='CGSize')
			return  "[$valueString  CGSizeValue]";
		
		if($nativeType=='CGPoint')
	...
	public function isScalar($nativeTypeName){
		if($nativeTypeName=='id'
				OR $nativeTypeName=='NSInteger' 
				OR $nativeTypeName=='NSUInteger' 
				OR $nativeTypeName=='float'
				OR $nativeTypeName=='double' 
				OR $nativeTypeName=='BOOL' 
				OR $nativeTypeName=='CGRect'
				OR $nativeTypeName=='CGPoint'
				OR $nativeTypeName=='CGSize'
				)
		 	return true;
		return false;
	}
