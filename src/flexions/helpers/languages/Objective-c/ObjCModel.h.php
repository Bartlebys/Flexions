		...
		if($property->instanceOf!=null){
			...
			if ($pos>=0 ){
				for ($i = 0; $i < 2; $i++) {
					...
					$collectionClassName=$instanceOf; // We qualify the $collectionClassName required by the collection template
					$tplExtension="h";
					if($i==1){
						$tplExtension="m";
					}
					// subTPL execution
					ob_start ();include "ObjCObjectCollection.$tplExtension.php";$subResult = ob_get_clean ();
