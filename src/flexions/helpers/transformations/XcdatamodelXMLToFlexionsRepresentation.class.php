	...
	function projectRepresentationFromXcodeModel($descriptorFilePath,$nativePrefix=""){
		...
		foreach ( $entities as $entity ) {
			...
			if($entity->hasAttribute("parentEntity") ){
				...
				// a requalification can be done according to the situation in the template
			}
			...
			foreach ( $attributes as $attribute ) {
				...
				if ($attribute->hasAttribute ( "name" )){
				 ...
				 $userInfo=$attribute->getElementsByTagName ( 'entry' );
				/* @var DOMNodeList userInfo */
				if( $userInfo->length > 0 && $userInfo->item(0)->hasAttribute ( "key" )  && rtrim($userInfo->item(0)->getAttribute ( "key" ))=="type" && $userInfo->item(0)->hasAttribute ( "value" )){
				/*
					 The user can define a specific type within the user info
					 Conventionnally we use a "type" as a key.
						
					<attribute name="myDictionary" optional="YES" attributeType="String" syncable="YES">
						<userInfo>
							<entry key="type" value="dictionary"/>
						</userInfo>
					</attribute>
				*/
					$propertyType=$userInfo->item(0)->getAttribute ( "value" );
					$property->type =$propertyType;
					
				}else if ($attribute->hasAttribute ( "attributeType" )){
